    ...
    if kind:
        ...
        if not perfect.issuperset(inPitches):  # must be subtraction or deletion....
            ...
            if additions:
                cs += ' add '
                for a in additions:
                    root = inChord.root().name
                    interval_from_root = interval.Interval(note.Note(root), note.Note(a))
                    if interval_from_root.semitones < 0:
                        interval_from_root = interval_from_root.complement
                    interval_from_root = interval_from_root.name
                    # todo add more
...
class ChordSymbol(Harmony):
    ...
    def _parseFigure(self):
        ...
        if 'add' in remaining:
            # TODO Warning
            ####### get_degree
            degree_and_alt = remaining[remaining.index('add') + 3:]
            # interval_from_root = interval.Interval(note.Note(root), note.Note(degree))
            # if interval_from_root.semitones < 0:
            #     interval_from_root = interval_from_root.complement
            # interval_from_root = interval_from_root.name
            # interval_to_degree_and_alteration = {
            #     'm7': (7, 0),
            #     'A2': (9, 1),
            #     'm2': (9, -1)
            # }
            ...
            if degree_and_alt[0] == '#':
                ...
                alt = 0
                degree = degree_and_alt


            self.addChordStepModification(ChordStepModification(
                'add',
                int(degree),
                alt)
            )
